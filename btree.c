...
static void node_append_range(node_t new_node, node_t old_node,
    ...
    for (uint16_t i = 0; i < n; i++) {
        set_node_ptr(new_node, dst_idx + i,
                     get_node_ptr(old_node, src_idx + i));
    }
...
static void node_replace_kid_n(btree_t *tree, node_t new_node, node_t old_node,
    ...
    for (uint16_t i = 0; i < n_kids; i++) {
        ...
        node_append_kv(new_node, idx + i, tree->create(kids[i]), key, key_len,
                       NULL, 0);
    }
...
static void node_merge(node_t new_node, node_t left, node_t right) {
    ...
    set_node_header(new_node, type,
                    get_node_nkeys(left) + get_node_nkeys(right));
...
static node_t node_delete(btree_t *tree, node_t node, uint16_t idx,
    ...
    case -1: {
        ...
        node_replace_2kid(new_node, node, idx - 1, tree->create(merged),
                          merged_key, merged_key_len);
        free(merged);
        break;
    }
    case 1: {
...
bool btree_delete(btree_t *tree, uint8_t *key, uint16_t key_len) {
    ...
    tree->del(tree->root);
    if (get_node_type(updated) == NODE_INTERNAL &&
        get_node_nkeys(updated) == 1) {
